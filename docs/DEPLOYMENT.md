...
Before replacing a running container, download a backup from Settings → Backup & about and keep a raw copy of the SQLite volume. Build the replacement image before stopping the old container, verify `/api/v1/health`, and retain the old image as a rollback tag.
